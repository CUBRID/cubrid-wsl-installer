...
**CUBRID For WSL Installer** is a Windows installer that deploys the CUBRID 11.4 database server onto Windows Subsystem for Linux 2 (WSL2) with a single click.
...
*   **One-click WSL2 deployment**
    ...
    * Imports a pre-built CUBRID image (`cubrid-wsl2-<version>.tar.gz`) into WSL2.
...
`build.bat` does **not** build the Linux image itself. It bundles a **pre-built CUBRID WSL2 image** (`cubrid-wsl2-latest.tar.gz`) that must already exist; during the build it is copied automatically from `make_image/targz_images/` into `os_image/`.
...
cd ..
```
*   `build_image.ps1` — clones `CUBRID/cubrid-docker` and builds the `cubrid-wsl2:11.4` Docker image (use `-Clean` for a fresh rebuild, `-Tags` for other versions).
*   `create_image.ps1` — exports the image's root filesystem to `make_image/targz_images/cubrid-wsl2-11.4.tar.gz` (and `cubrid-wsl2-latest.tar.gz`), which `build.bat` then consumes.
...
*   **Build (default)**
 ...
 The build artifact (`CUBRID-<cubrid-version>-For-WSL-<cubrid-version>-<product-version>-<git-revision>-win64.exe`) is generated in the `build` directory at the project root.
