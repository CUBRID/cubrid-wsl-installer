...
### 2. 메이저 / 마이너 버전 마이그레이션 (DB 볼륨 비호환)

서로 다른 메이저 버전 사이에서는 DB 볼륨 형식이 호환되지 않을 수 있습니다. 이런 경우에는 `cubrid unloaddb` 와 `cubrid loaddb` 유틸리티를 이용하여 데이터를 텍스트 파일로 내보낸 뒤, 신규 버전의 CUBRID 에서 다시 가져와야 합니다.
