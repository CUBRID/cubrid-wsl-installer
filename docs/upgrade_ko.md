...
* 서로 다른 **주(major) 버전** 사이에서는 DB 볼륨이 호환되지 않을 수 있으므로 `unloaddb` / `loaddb` 를 통한 마이그레이션이 필요합니다.
...
| **마이너 / 메이저 버전 변경** | x.3 → x.4, x.0 → y.0 | `unloaddb` / `loaddb` 로 데이터 마이그레이션 |
...
동일한 주 버전 안에서의 업그레이드(예: x.x.0 → x.x.1)는 DB 볼륨 형식이 호환되므로 **설정 파일만 보존**하면 됩니다.
